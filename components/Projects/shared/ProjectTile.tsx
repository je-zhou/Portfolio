...
export default function ProjectTile({
  ...
  return (
    ...
    >
      <div
        ref={ref}
        className="flex flex-col items-start justify-start w-full space-y-4 max-w-md md:max-w-md py-4 md:py-0"
      >
        {/* Title and Logo */}
        <div className="flex flex-row items-center space-x-2">
          <div className="w-8 h-8 bg-white border rounded-md relative flex items-center justify-center shadow-xs">
            <div className="w-6 h-6 relative">
              <Image src={logoUrl} fill={true} alt={logoUrl}></Image>
            </div>
          </div>
          <h4 className="text-xl font-bold text-charade dark:text-white">
            {name}
          </h4>
        </div>
        {/* Tags */}
        <div className="flex flex-row space-x-2">
          {tags.map((tag) => {
            return <TagTile key={tag} tag={tag}></TagTile>;
          })}
        </div>
        {/* Description */}
        ...
        >
          <p className="font-bold text-lg text-charade dark:text-white">
            Find out more
          </p>
          <FontAwesomeIcon
