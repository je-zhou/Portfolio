...
export default function InfoTile({ title, description, display }: InfoTileProps) {
	return (
		<div className='flex flex-col space-y-4 w-full py-20 items-center md:items-start'>
			...
			<div className='flex flex-col items items-center md:items-start w-full md:flex-row space-y-8 md:space-y-0'>
				{/* Description */}
				<div className='max-w-sm text-charade dark:text-white text-center md:text-left font-light space-y-2'>
					{description.split('\n').map(str => (<p key={str}>{str}</p>))}
				</div>
				{/* Display Component */}
				<div className='flex-1 w-full flex justify-center md:pl-16'>
					{display}
				</div>
			</div>
		</div>
